...
- `cli.py` - 命令行接口实现，包含参数解析和处理功能，支持从配置文件加载和组织化参数显示。提供丰富的超参数配置选项，包括损失函数类型、各种优化器特定参数（如SGD的momentum、Adam的beta值等）和学习率调度器参数（如cosine的t-max、eta-min等）
...
3. 命令行接口流程：
   ...
   - CLI支持多种超参数配置，包括损失函数类型、所有主流优化器的特定参数和多种学习率调度策略的参数设置
